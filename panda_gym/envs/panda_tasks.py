...
from panda_gym.envs.robots.panda_ik import PandaIK
...
class PandaPushSafetyDaggerEnv(RobotTaskEnv):
    """Push task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, -0.2]), control_type=control_type)
        task = Push(sim, reward_type=reward_type)
        super().__init__(
            robot,
            task,
